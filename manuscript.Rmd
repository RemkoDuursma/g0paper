...
Within the model framework, g~0~ is reached when photosynthesis (A~n~) goes to zero. Based on this, it is frequently assumed that g~0~ must equal g~s~ at night, simply because no photosynthesis occurs at night [@lombardozzi2016; @barnard2013; @uddling2005]. It is also frequently assumed that g~0~ is the 'cuticular conductance` [@baldocchi1997; @manzoni2011; @egea2011], which technically is the conductance of the cuticle alone - ignoring leaky and incompletely closed stomata [@kerstiens1996]. Another measurement of the minimum conductance is the leaf drying curve: leaves are picked, and gradually dried out and weighed to determine the water loss rate [@hygen1951]. This estimate of minimum conductance (which we refer to as g~min~) probably most closely reflects the dessiccation dynamics of severely droughted plants [@blackman2016], but has rarely been mentioned in the context of the Ball-Berry model. 

In this review, we aim to compare values of the various estimates of minimum conductance, discuss components and mechanisms of minimum conductance. We place additional emphasis on measurements of g~min~, sources of variation in g~min~, and the role that g~min~ can play in models of water use. Before we discuss the various definitions and measurements of minimum conductance, we first review the modelling literature on its use of g~0~ in Eq. 1 (and related approaches), and demonstrate why g~0~ needs to be included in models.
...
where a = V~cmax~ or ~J/4~ (where J goes to Jmax at high light) depending on the limitation to photosynthesis, and b also varies. The dependence on temperature enters through V~cmax~, J~max~, R~d~, $\Gamma^{*}$, and other parameters [@medlyn2002]. 
