...
- bel looks for data on low PAR; maybe replace gslowA with gslowPAR, so we can argue that g0 is much higher than gslowPAR. 
- some data behind figure 4a (and maybe Arnaud 4d?)
- min(g0, gs) ('Uddling', has he used it?)  vs. g0 + gs.



We could say that plant dilemma is to be both impermeable to water loss (because they are saturated with water and leave are in an extremely dry air environment and hence need a external layer to prevent dessication= cuticle) AND need to be permeable to CO2 to grow. The dilemma is that leaves cannot be permeable to CO2 and not to H20. Hence the evolution of stomata that disrupt the cuticle layer enabling CO2 entre but cannot avoid some CO2 exit but can also close to limit H20. loss.

  It could be interested to discuss somewhere in the text the relevance of the conductance (gmin) versus the flow (Emin). Obviously gmin is more an intrinsic property of the leaf but what matters for the leaf water relation of the flow. In terms of drought resistance and water balance (and water relations) Ecuti is more relevant than gmin I think. At night for instance gdark is high but vpd so the flow might not be so different than at gmin during the day.
   
finding of Schuster e al (2016) who found a high proportion of triterpenoid in the cuticule of a desertic thermotolerant species. This compound would act as to stabilize the cuticule.
...
\
A number of studies have assumed that the g~0~ in the Ball-Berry model equals the night-time conductance (g~dark~) [@barnard2013; @lombardozzi2017], simply because it is a condition when net photosynthesis is zero. However as our literature comparison showed (Fig. 1), g~dark~ is substantially higher compared to all other variables except fitted g~0~. The simple idea that g~dark~ is equal to the minimum reached during periods of near-zero PPFD or high temperature during the day can be dismissed based on several observations.
...
$$ g_s = min(g_{min}, g_0) + g_1 \frac{A_n}{C_a}f(D)$$
