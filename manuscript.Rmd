...
II. Use of minimum conductance in models
\t1. Models of water-use efficiency
\t2. Models of plant desiccation
\t3. Towards a new model formulation
III. Comparison of various definitions of minimum conductance
IV. Problems with estimating Minimum conductance from regression
V. Cuticular conductance
VI. Contribution of stomata
...
\
The absolute minimum attainable water loss rate is through the cuticle only, which is typically measured on isolated cuticles of the adaxial (non-stomatal) side of the leaf. Other measurements allow for the estimation of water loss through incompletely closed and broken stomata. Of particular interest because of its simplicity is the method where leaves are detached from the plant, and their weight loss monitored over time. This method (mass loss of detached leaves, MLD) aims to simulate field conditions during severe drought, when water supply to the leaf has practically ceased, however proceeds much quicker (typically 0.5-2 days). This measurement also includes both surfaces of the leaf - not just the stomata-free side. A comprehensive compilation of estimates of g~min~ is currently lacking, and is needed not just to parameterize models, but also to study sources of variation in this overlooked plant trait.

\
In this review, we discuss the role of the minimum conductance in models of plant water use, and critically evaluate sources of variation in this overlooked parameter. Previous reviews of the minimum conductance have largely focused on the biology of the plant cuticle, and detailed physiology and anatomy of water transport across cuticles [@riederer2008, @schuster2017; @kerstiens1996; @fernandez2017]. Other work has focused on a different definition of the minimum conductance (g~0~): the value that should be used in models of plant water use [e.g. @barnard2013; @lombardozzi2017], and includes both g~min~ and a 'stomatal residual' due to the fact that stomata do not completely close during periods of zero photosynthesis [@dekauwe2015]. This discussion often centers around leaf conductance during the night [@lombardozzi2017], but we must also consider appropriate values for the minimum conductance during the day, for example during very high (or low) temperature, transient low light, and extreme drought. Thus there is a disconnect between the dizzying level of understanding of the plant cuticle [@riederer2008], and the very simple assumptions made when using leaf conductance models in global vegetation models.

\
We aim to improve the connection between the physiology of minimum conductance, empirical approaches, and model implementations by synthesizing the state of knowledge. Our goals are to a) demonstrate the need to include a non-zero minimum conductance in models of water-use efficiency and drought responses, and discuss current practices b) compare various definitions of g~min~ quantitively, which have sometimes been previously assumed equal, c) present a new compilation of minimum conductance measurements, test for ecologically meaningful patterns, and demonstrate the large acclimatory potential of this trait to environmental drivers. 
...
\
Using the coupled leaf gas exchange model, we show in Fig. 1 how A~n~/g~s~ and C~i~ depend on PPFD with three values of g~0~ (0, 0.01 and 0.03 mol m^-2^ s^-1^). We also demonstrate the effect of g~0~ on modelling the T~leaf~ response. In this simulation, T~leaf~ and VPD are assumed to co-vary with an empirical relationship as used by @duursma2014. Over the entire range of T~leaf~, inclusion of a non-zero g~0~ obviously increases leaf transpiration, but at a slightly higher rate than just due to g~0~. This effect arises because the additional conductance allows slightly higher rates of photosynthesis, which in turn increase g~s~ via Eq. 1. 
...
\
We contrasted g~cuti~ and g~min~ with three estimates of stomatal conductance when photosynthesis rates are low or zero during non-drought conditions: night-time conductance (g~dark~) (further discussed in Section VII), conductance at low photosynthetically active radiation (PAR) (0 - 40 $\mu$ mol m^-2^ s^-1^) during the day, and conductance during conditions when photosynthesis rates are very low (< 1 $\mu$ mol m^-2^ s^-1^) (but excluding low PAR and drought). 
...
\
In the following sections, we review the quite substantial literature on g~min~ measured with the mass loss of detached leaves (MLD) technique, stretching back to the 1930s [@pisekberger1938], focussing on environmental and ecological determinants. We also further analyze the literature compilation of g~min~ (Fig. 6), and separately analyze crop species and their genotypic variation. Comparing all available data, we did not find significant relationships between g~min~ and climate of origin, nor were there meaningful relationships with other traits (see Methods S1). We thus found it difficult to explain the variation in g~min~ between species. A simple breakdown by taxonomic order (Fig. 6b) revealed that grasses (*Poales*) have a higher g~min~ compared to other orders, and conifers (especially *Pinales*) tended towards the lower range of values (but were only significantly different from *Poales*).

\
Very few studies have found meaningful correlations between g~min~ and environmental factors or ecological categories. Based on their own compilation of the literature, @schuster2017 concluded no significant differences in g~min~ or g~cuti~ (analyzed together) by plant growth form. One notable exception is @brodribb2014, who showed a correlation between rainfall at species origin (of the driest quarter), but used a gas-exchange approach (over many weeks), not MLD. Such correlation did not bear out across all species in our database, and interesting to note is that *Eucalyptus* species (n = 11, included in *Myrtales*) - all measured on the driest continent of Australia - have g~min~ values slightly higher than the average (Fig. 6b).
