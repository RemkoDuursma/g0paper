...
figure_sureau <- function(planta, plantb){
  ...
  # panel a
  with(planta, plot(Days, REW, lwd=2, ylim=c(0,1), xlim=c(0,80), type='l', xlab="Days", col=cols[1]))
  with(plantb, lines(Days, REW, lwd=2, ylim=c(0,1), xlim=c(0,80), col=cols[2]))
  plotlabel("(a)", "bottomright")
  legend("topright", c("2", "4"), title=expression(g[min]), bty='n', lty=1, col=cols, lwd=2, cex=1.2)
  
  # panel b
  with(planta, plot(Days, P_leaf_symplasmic, ylim=c(-7,0), xlim=c(0,80), 
                    col=cols[1],
                    ylab= "Water potential (MPa)",
                    type='l', lwd=2, lty=1, xlab="Days"))
  with(planta, lines(Days, P_soil, lwd=2, col=cols[1], lty=5))
  with(plantb, lines(Days, P_leaf_s, type='l', lwd=2, lty=1, col=cols[2]))
  with(plantb, lines(Days, P_soil, lwd=2, col=cols[2], lty=5))
  plotlabel("(b)", "bottomright")
  legend("topright", c("leaf","soil"), lty=c(1, 5), lwd=2, bty='n', cex=1.2)
  
  # panel c
  with(planta, 
       plot(Days, PLC_branch, xlim=c(0,80), 
            ylab="PLC (%)", type='l', lwd=2, 
            xlab="Days", col=cols[1],
            panel.first= abline(h = 88, lty=2, col="darkgrey"))
  )
  with(plantb, lines(Days, PLC_branch, xlim=c(0,80), type='l', lwd=2, col=cols[2]))
